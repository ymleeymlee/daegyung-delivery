...
export default function RiderAddModal({ riderName, onPick, onClose }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-[9999] p-4 pt-24" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-[34rem] max-w-full max-h-[80vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-3 flex flex-col gap-3 min-h-0">
          ...
          <div className="flex gap-3 min-h-0">
            {/* 검색 결과 리스트 (좌측) */}
            <div className="flex-1 min-w-0 overflow-y-auto max-h-[52vh] flex flex-col gap-1">
            ...
            {/* 직접 그린 숫자 키패드 (우측). OS 터치 키보드 대신, 하드웨어 키보드 숫자도 입력됨 */}
            <div className="w-44 flex-shrink-0 grid grid-cols-3 gap-2 self-start" onMouseDown={e => e.preventDefault()}>
